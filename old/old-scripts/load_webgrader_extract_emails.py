...
extract_email_csv_directory = '/mnt/emc/Import/be/new/WebGraderSnappyExport/results-scan-emails/'
...
def load_cvs_into_webgrader_extract_email(file_name):
    ...
    # remove long emails
    df = df[df["contact_value"].notna() & (df["contact_value"].str.len() <= 200)]
    df.to_sql("webgrader_extract_email", engine, if_exists="append", index=False)

with engine.begin() as conn:
    conn.execute(text("truncate table webgrader_extract_email"))
    conn.commit()

print("Scan for CSVs.")
for subdir, dirs, files in os.walk(extract_email_csv_directory):
        for file in files:
            if file.endswith(".csv"):  # Kontrola, zda je to zip soubor
                load_cvs_into_webgrader_extract_email(f"{subdir}/{file}") 
