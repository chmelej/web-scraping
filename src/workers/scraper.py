...
from config.settings import SCRAPE_DELAY, USER_AGENT, MAX_RETRIES, PLAYWRIGHT_HEADLESS

class Scraper:
    def __init__(self):
        self.logger = setup_logging('scraper', 'scraper.log')
        try:
    ...
    def get_next_url(self):
        ...
        try:
            ...
            self.logger.error(f"Error getting next URL: {e}")
            # Reconnect
            try:
                self.conn = get_db_connection()
            except:
        ...
        try:
            with get_cursor(self.conn, dict_cursor=False) as cur:
                ...
                """, (
                    ...
                    scrape_result.get('headers'), scrape_result.get('ip_address'),
    ...
    def add_subpages_to_queue(self, parent_id, parent_url, html, language, unit_listing_id, depth):
        ...
        try:
            ...
            self.logger.error(f"Error adding subpages: {e}")
            self.conn.rollback()
    ...
    def update_queue_status(self, queue_id, status, retry_count=None, next_scrape_at=None):
        ...
        try:
            with get_cursor(self.conn, dict_cursor=False) as cur:
             ...
             self.logger.error(f"Error updating queue: {e}")
             self.conn.rollback()
    ...
    def process_one(self):
        ...
        if result['status_code'] == 200 and result['html']:
            ...
            # Add sub-pages
            if unit_listing_id and scrape_result_id:
